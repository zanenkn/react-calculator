...
describe('Function updateDisplay', () => {
  let wrapper
  beforeEach(() => wrapper = shallow(<Calculator />))
...
describe('Function setOperator', () => {
  ...
  it('if selectedOperator is not an empty string, does not update storedValue', () => {
    wrapper.setState({ displayValue: '5' })
    wrapper.instance().setOperator('+')
    expect(wrapper.state('storedValue')).toEqual('5')
    wrapper.instance().setOperator('-')
...
describe('Function callOperator', () => {
  let wrapper
  beforeEach(() => wrapper = shallow(<Calculator />))
